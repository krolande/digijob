...
type UsersDigicodeuses struct {
	...
	IdC             int
	Libelle         string
	TabLibelle      []entities.CompetenceCles
	CreatedAtC      string
	UpdatedAtC      string
}
...
type UsersDigi struct {
	...
	Prenoms         string         `validate:"required,prenoms,isunique=users-prenoms"`
	Email           string         `validate:"required,email,isunique=users-email"`
	Username        sql.NullString `validate:"required,gte=3,isunique=users-username"`
	Pays            string         `validate:"required,pays,isunique=users-pays"`
	Ville           string
...
// type CompetenceClesDigi struct {
// 	Id         int
// 	Libelle    string
// 	TabLibelle []entities.CompetenceCles
// 	CreatedAt  string
